...
> **说明：** GB/T 9704-2012 的参考主要针对“公文类”中的传统公文模板。论文类、周报类、活动类和培训类属于 FORGE 的业务模板规范，不应理解为全部属于国家标准公文格式。FORGE 也不替代最终的人工审校或具体单位内部行文制度。

### Current formatting rules / 当前内置格式要求

| 文档类别 | 当前主要版式规则 |
|---|---|
| **论文类** | 适用于论文 / 演讲稿 / 发言稿 / 长文。无页眉；标题和各级标题默认不加粗；正文小四宋体、固定 18 磅；支持摘要、关键词、三线表和图片；演讲稿 / 发言稿可省略摘要和关键词。 |
| **公文类** | 适用于传统公文 / 计划 / 总结 / 方案 / 汇报 / 报告 / 请示。标题采用 2 号方正小标宋、居中、固定 32 磅；正文采用 3 号仿宋、固定 28 磅；支持“一、→（一）→1.”三级层次，分别使用黑体 / 楷体 / 仿宋；页码为 4 号宋体阿拉伯数字并带一字线；普通公文不使用红色红头和双红线。 |
| **活动类** | 传统公文活动方案 / 活动方案沿用公文正文体系，并支持“单位 + 部门 + 日期”三行落款；活动总结使用结构化总结版式；活动影像使用“标题 + 信息表 + 图片页”结构，可将两张照片排在同一页。 |
| **周报类** | 适用于行政周报 / 周报。使用两行标题，按部门分节，每段自动编号；没有内容时可输出“无”。 |
| **培训类** | 培训通知使用通知类专用格式，包含红色红头与双红线；培训活动记录使用结构化记录表格；培训活动影像使用照片页版式。 |

FORGE 的关键设计原则是：**模板才是格式的唯一事实来源（source of truth）**。字体、字号、行距、页边距、标题层级、落款位置、表格、图片区域、页码等尽量由模板和固定规则控制，而不是交给大模型临时决定。

## Supported target types / 支持的目标类型（全版本）

当前版本面向用户提供 **5 类正式目标类型**。你可以直接用这些名称描述想要生成的材料，FORGE 会将目标类型映射到对应模板和结构规则。

- **论文类**：论文 / 演讲稿 / 发言稿 / 长文
- **公文类**：传统公文 / 计划 / 总结 / 方案 / 汇报 / 报告 / 请示
- **活动类**：传统公文活动方案 / 活动方案 / 活动总结 / 活动影像
- **周报类**：行政周报 / 周报
- **培训类**：培训通知 / 培训活动记录 / 培训活动影像

### Target mapping / 目标类型与模板能力

| 类别 | 正式目标类型 | 主要用途 / 模板能力 |
|---|---|---|
| **论文类** | `论文`、`演讲稿`、`发言稿`、`长文` | 长篇文字材料；支持摘要、关键词、三级结构、三线表与图片 |
| **公文类** | `传统公文`、`计划`、`总结`、`方案`、`汇报`、`报告`、`请示` | 通用正式公文材料；共享传统公文模板与标准层级规则 |
| **活动类** | `传统公文活动方案`、`活动方案`、`活动总结`、`活动影像` | 覆盖活动前期方案、过程资料、总结与影像归档 |
| **周报类** | `行政周报`、`周报` | 按部门组织的周期性行政周报 |
| **培训类** | `培训通知`、`培训活动记录`、`培训活动影像` | 覆盖培训通知、过程记录与影像归档 |

> **兼容说明：** 代码中仍保留 `培训通知记录`、`通知培训资料` 等历史兼容入口，用于旧调用方式；新的 AI / MCP 工作流建议优先使用上面的正式目标类型或“成套资料”能力。`sample` 仅用于验证 MCP 服务与模板渲染，不属于正式业务目标类型。

### Document sets / 成套资料

除了单份文档，FORGE 还可以一次生成一组相互配套的材料：

- **活动方案套装**：活动方案 → 活动总结 → 活动影像
- **培训资料套装**：培训通知 → 培训活动记录 → 培训活动影像

这意味着用户可以直接提出“生成一套活动资料”或“生成一套培训资料”，由 FORGE 按既定顺序生成多个相互对应的 DOCX，而不需要逐份手工调用。
...
1. **识别目标类型**：先判断需求属于论文类、公文类、活动类、周报类还是培训类，再在该类别中选择具体目标类型，例如“总结”“活动影像”或“培训通知”。
...
| Tool | Purpose / 用途 |
|---|---|
| `list_document_types` | 查看当前可用目标类型、模板、格式指南和文档套装 |
| `get_template_schema` | 查看指定模板所需字段、说明和示例数据 |
| `get_semester_info` | 获取学年度 / 学期辅助信息 |
| `recommend_document_type` | 根据用户需求推荐合适目标类型 |
| `generate_docx` | 按指定模板直接生成 DOCX |
| `generate_by_type` | 按友好目标类型生成 DOCX |
